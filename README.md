...
- 위협 오버레이 · 이벤트 카드. 안티 웨이브 / 장비 렉은 0.5초 전에 `Art/event_warn` 경고 플레이트 **안티 온다** / **렉 온다** 가 뜬 뒤, 실제 발화 때 `Audio/sfx_anti` 야유 + `Art/anti_sting` 빨간 야유 오버레이 / `Audio/sfx_lag` 글리치 + `Art/lag_sting` 정적 오버레이. **이벤트 경고**는 그 라이브 **안티 온다** / **렉 온다** + **아침 경고**(**오늘의 위협** 슬램) + 정산 추가 위협 줄(없으면 숨김) + **이어서 하기** 추가 위협(세이브·위협 없으면 숨김)이 같은 `Art/event_warn`. 청구를 넘기는 순간 `Art/bill_cover` 금색 PAID 스탬프 **청구 커버** 슬램(한 판 한 번, 이후 초록 고정) + `Audio/sfx_bill_cover` 캐시 레지스터 스팅.
