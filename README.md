...
- **이벤트 경고 플레이트** — 안티/렉 0.5초 전 **안티 온다** / **렉 온다** 가 `Art/event_warn` 방송 경고 플레이트 (타이밍·카피·규칙 그대로)
- **이벤트 스팅 오버레이** — 발화 시 `Art/anti_sting` 빨간 야유 · `Art/lag_sting` 정적 오버레이 (`sfx_anti` / `sfx_lag` · 텔레그래프·규칙 그대로)
