...
- **라이브 HUD 스택** — 라이브 한 판에서 보이는 오버레이·스탬프·칩(히트창·콤보·하이프·이벤트·경제 숫자는 그대로).
  ...
  - **노트 레인** — `Art/note_lane` — 네 갈래 노트 경로 레인 베드(패드 색 ←파랑 / ↓초록 / →트롤 / ↑골드. `Art/hit_rail` 스트라이크 포켓은 그대로)
...
- **패드 / 채팅 / 노트** — 라이브 키캡·버블·닉 네임플레이트가 이미 붙어 있다(스폰·카탈로그·경제 숫자는 그대로).
  ...
  - `Art/note_lane` — 노트 경로 레인 베드(패드 색 ←파랑 / ↓초록 / →트롤 / ↑골드. `note_chip`·`superchat_chip` 아래. `hit_rail` 스트라이크는 그대로)
