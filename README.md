...
- **라이브 HUD 스택** — 라이브 한 판에서 보이는 오버레이·스탬프·칩(히트창·콤보·하이프·이벤트·경제 숫자는 그대로).
  ...
  - **피크 / 사고** — `Art/hype_frame`(하이프 골드 프레임, `sfx_hype`) · `Art/hype_chip`(**하이프 N**) · `Art/event_warn` — **아침 경고**(**안티 온다** / **렉 온다** · 아침 **오늘의 위협** 슬램) · `Art/anti_sting`(`sfx_anti`) · `Art/lag_sting`(`sfx_lag`)
