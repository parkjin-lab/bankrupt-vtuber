...
- **라이브 HUD 스택** — 라이브 한 판에서 보이는 오버레이·스탬프·칩(히트창·콤보·하이프·이벤트·경제 숫자는 그대로).
  ...
  - **피크 / 사고** — `Art/hype_frame`(하이프 골드 프레임, `sfx_hype`) · `Art/hype_chip`(**하이프 N**) · `Art/event_warn` — **이벤트 경고**(**안티 온다** / **렉 온다** · **아침 경고** **오늘의 위협** 슬램 + `sfx_threat` 한 번 · 정산 **위협** 줄 · **이어서 하기** 추가 위협, 세이브·위협 없으면 숨김) · `Art/anti_sting`(`sfx_anti`) · `Art/lag_sting`(`sfx_lag`)
...
- **얼굴 / 아이콘** — 웹캠 얼굴·베젤·쇼 아이콘이 이미 붙어 있다(펀치·지침·라이벌 규칙·경제 숫자는 그대로).
  ...
  - `Art/rival_nyang` — 3주차 **라이벌 HUD**. `Art/webcam_bezel` · 듀얼 중 `Art/onair_led`(승패 나면 꺼짐) · 카운트 `Art/viewer_badge` · ± 팝 `Art/viewer_pop`
