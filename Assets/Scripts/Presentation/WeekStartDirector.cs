...
namespace BankruptVtuber
{
    public class WeekStartDirector : MonoBehaviour
    {
        ...
        void Build()
        {
            ...
            var hint = UiKit.Label(root, "Hint", "A 긍정   S 공감   D 웃음   F 감사   Space 슈퍼챗", 18, Palette.Muted, TextAnchor.LowerRight);
        ...
        IEnumerator BillWave(GameManager gm)
        {
            ...
            var bills = new[]
            {
                new Bill { Name = "월세", Icon = "집", Amount = b.billRent },
                new Bill { Name = "전기+인터넷", Icon = "번개", Amount = b.billElectricNet },
                new Bill { Name = "아바타 라이선스", Icon = "얼굴", Amount = b.billAvatarLicense },
                new Bill { Name = "식비", Icon = "밥", Amount = b.billFood },
                new Bill { Name = "장비 할부", Icon = "마이크", Amount = b.billGear }
            };
        ...
        void SpawnIncoming(Bill bill, int index, int total)
        {
            ...
            UiKit.Layout(card, new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(x, 220), new Vector2(170, 190));
            UiKit.Label(card, "Icon", bill.Icon, 28, Palette.PinkDeep, TextAnchor.UpperCenter, FontStyle.Bold);
            var ic = card.Find("Icon") as RectTransform;
            UiKit.Layout(ic, new Vector2(0.5f, 1), new Vector2(0.5f, 1), new Vector2(0.5f, 1), new Vector2(0, -12), new Vector2(140, 36));
            UiKit.Label(card, "N", bill.Name, 20, Palette.Ink, TextAnchor.MiddleCenter, FontStyle.Bold);
            var n = card.Find("N") as RectTransform;
            UiKit.Layout(n, new Vector2(0, 0.45f), new Vector2(1, 0.7f), new Vector2(0.5f, 0.5f), Vector2.zero, Vector2.zero);
            var amt = UiKit.Label(card, "A", "-" + EconomyRules.FormatWon(bill.Amount), 22, Palette.MoneyRed, TextAnchor.LowerCenter, FontStyle.Bold);
            UiKit.Layout(amt.rectTransform, new Vector2(0, 0), new Vector2(1, 0.38f), new Vector2(0.5f, 0.5f), Vector2.zero, Vector2.zero);
